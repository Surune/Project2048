...
int main()
{
	...
	scan_ID_1(65); 									 
	scan_ID_2(65);
	scan_ID_3(65);                                  // ID(이름) 입력받음
	print_loading(10);
	tutorial(5);
	print_head();
	make_random();
	make_random();									// 랜덤한 위치에 숫자 2개 생성 
	printf("\n");
...
int tutorial(int n)
{
	...
	printf("│                                  │\n");
	printf("│         ");
    YELLOW printf("[ 조작키 설명 ]");
    ORIGINAL printf("          │\n");
	printf("│                                  │\n");
	printf("│  ↑: W   ↓: S   ←: A   →: D   │\n");
	printf("│                                  │\n");
    printf("│   찬스: C   UNDO: U   REDO: R    │\n");
	printf("│                                  │\n");
    printf("│    AI: P   재시작: N   종료: X   │\n");
	printf("│                                  │\n");
	printf("└─────────────────┘\n\n");	
	MAGENTA printf("          %d초 후 사라집니다.         ", n);
	ORIGINAL printf("\n");
	Sleep(1000);
	if(n==0)   return 0;
	else       return tutorial(n-1);
}
...
{
	MAGENTA printf("\n ID : %c%c%c SCORE : %5d점 STAGE : %3d", player[4][0], player[4][1], player[4][2], player[here][3], player[here][4]);
    ORIGINAL printf("\n");	
	for (int i = 1; i<5; i++)
	{
		if (i == 1)
...
void move()
{
	switch (dir)									// 어느 방향으로 밀것인지 입력받기
	{
		...
		case 'c':									// 찬스를 입력받은 경우
			...
			{
				RED printf("        CHANCE : 남은 횟수 %d회", player[4][5]-1);
				ORIGINAL printf("\n");
				chance();							// 찬스 사용 
			}
			else									// 찬스를 다 사용했다면 
			{
				RED printf("      CHANCE를 다 사용하셨습니다.");
				ORIGINAL printf("\n");
			}
			print_field();       					// 계산된 값 출력
			break;
...
void repeat()										// 반복할 함수(게임의 기본 함수) 
{
	...
	if((dir == 'N')||(dir == 'n'))	    main();	
	else if((check() == 2)||(check() == 0)||(dir == 'X')||(dir == 'x'))	// 게임이 끝날 조건일 경우 
	{
		ending();
		return;
	}
	else if((dir=='P')||(dir=='p'))	   ai();
	else
    {   
        move();
        repeat();
