...
---------------------------------- [4단계 완성]---------------------------------- 
2016.11.26
ai 함수 추가 : 인공지능을 실행하는 함수 
move 함수 추가 및 repeat 함수 수정 :  ai를 위하여 move 함수 분리 

2016.11.27
tutorial 함수 추가 및 print_head 함수 수정 : 직관성을 위하여 방향키 설명 추가 
주석 추가 
전체적 직관성 

2016.12.02
player 배열-> 구조체로 변경하여 사용 
*/
...
void new_game()										// 변수 초기화 
{
	...
	for (i = 0; i < SIZE; i++)
		for (j = 0; j < SIZE; j++)
			for(k = 0; k < SIZE ; k++)
				new_player.field[k][i][j] = 0;		// 숫자판 초기화 

	for (i =0; i < 5; i++)
	{
		new_player.score[i]=0;								// 점수 초기화 
		new_player.stage[i]=1;                    			// 단계 초기화 
		new_player.chance[i]=3;							// 찬스 남은 횟수 초기화 
	}
}
...
void tutorial()										// 방향키 설명해주는 함수 
{
	...
	ORIGINAL printf("│\n");
	printf("│                                  │\n│");
	MAGENTA printf("   찬스: C   UNDO: U   REDO: R    ");
	ORIGINAL printf("│\n");
	printf("│                                  │\n│");
	MAGENTA printf("  자동: P   재시작: N   종료: X   ");
	ORIGINAL printf("│\n");
	printf("│                                  │\n");
	printf("└─────────────────┘\n");	
	Sleep(5000);
...
{
	int i, j;
	MAGENTA printf("\n ID : %c%c%c SCORE : %5d점 STAGE : %3d", new_player.name_1, new_player.name_2, new_player.name_3, new_player.score[4], new_player.stage[4]);
    ORIGINAL printf("\n");	
	for (i = 1; i<5; i++)
	{
		if (i == 1)
...
{	
	...
	for (i = 0; i < SIZE; i++)
	{
		for (j = 0; j < SIZE; j++)
		{
			copy[i][j] = new_player.field[4][i][j];	// copy 배열에 복사 
		}
	}
	...
	for (i = 1; i < SIZE - 1; i++)
	{
		for (j = 1; j < SIZE - 1; j++)	
		{
			if (copy[i][j] != new_player.field[4][i][j])	// 움직이기 전과 움직인 후가 다르다면
				check = 1;							// check에 1 저장
		}
	}
...
void chance()										// 찬스 함수 
{
	...
	new_player.chance[4]--;							// 찬스 남은 횟수 차감 
}

void memory()										// 정보를 밀어 올리는 함수 
{
	int i, j, k;
	...
	for(i=0; i<SIZE; i++)
	{
		...
		{	
			for(k = 0; k < 4; k++)
			{
				new_player.field[k][i][j]= new_player.field[k+1][i][j];	// 배열 밀어 올리기 
			}
			
			new_player.field[4][i][j]=copy[i][j];			// 배열 옮겨 담기 
		}
	}
	...
	for(k=0; k<4; k++)
	{
		...
		new_player.chance[k]=new_player.stage[k+1];	// 플레이어 정보 밀어 올리기
	}
}
...
void clear()										// 모서리를 초기화 시키는 함수 
{
	...
	for (i = 0; i < SIZE; i++)
	{
		new_player.field[4][i][0] = 0;							// 각 모서리 초기화 (쓰레기값 방지)
		new_player.field[4][i][SIZE-1] = 0;					// 각 모서리 초기화 (쓰레기값 방지)
	}

	for (j = 0; j < SIZE; j++)
	{
		new_player.field[4][0][j] = 0;							// 각 모서리 초기화 (쓰레기값 방지)
		new_player.field[4][SIZE-1][j] = 0;					// 각 모서리 초기화 (쓰레기값 방지)
	}
}
...
void make_random()									// 랜덤으로 숫자를 만들어주는 함수
{
	...
	for(i=1; i<SIZE-1; i++)
	{
		for(j=1; j<SIZE-1; j++)
		{
			if(new_player.field[4][i][j]==0) n++;				// 게임판의 0의 개수 확인
		}
	}
	...
	for(i=1; i<SIZE-1; i++)
	{
		for(j=1; j<SIZE-1; j++)
		{
			if(new_player.field[4][i][j]==0)					// 0의 위치에만 새로운 숫자를 넣어야 한다. 
			{
				if(rand()%n ==0)					// 1/n의 확률로 이 위치에 새로운 숫자가 들어간다. 
				{
					if(rand()%5 == 4) 
						new_player.field[4][i][j]=4;			// 20%의 확률로 4
					else 
						new_player.field[4][i][j]=2;			// 80%의 확률로 2
...
void move()											// dir에 따라서 숫자판을 움직여주는 함수 
{
	switch (dir)									// 어느 방향으로 밀것인지 입력받기
	{
		...
		case 'u':
			print_head();			
			if((here<1)||(new_player.stage[here]<2))		// 되돌릴 수 없는 경우 
			{
